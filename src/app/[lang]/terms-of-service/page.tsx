...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return {
    title: `${t.termsOfService} - Best Call Time`,
    description: 'Terms of Service for Best Call Time.',
  };
}
  ...
  return (
    <main className="w-full pt-16 pb-24 bg-surface min-h-screen">
      <div className="max-w-3xl mx-auto px-6 lg:px-8">
        ...
        <h1 className="font-display-lg text-headline-xl text-on-surface mb-6">{t.termsOfService}</h1>
        <div className="prose prose-slate max-w-none text-on-surface-variant font-body-md space-y-6">
          <p>{tPage.termsUpdated}</p>
          <p>{tPage.termsP1}</p>
          
          <h2 className="text-headline-md font-semibold text-on-surface mt-8 mb-4">{tPage.termsH1}</h2>
          <p>{tPage.termsP2}</p>
          
          <h2 className="text-headline-md font-semibold text-on-surface mt-8 mb-4">{tPage.termsH2}</h2>
          <p>{tPage.termsP3}</p>
          
          <h2 className="text-headline-md font-semibold text-on-surface mt-8 mb-4">{tPage.termsH3}</h2>
          <p>{tPage.termsP4}</p>
        </div>
      </div>
    </main>
